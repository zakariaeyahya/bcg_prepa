...
\rhead{\small BCG X CodeSignal Prep}\lhead{\small Livraison de repas}
\cfoot{\thepage}
...
\begin{center}
  {\huge\bfseries\color{bcg} Livraison de repas}\\[4pt]
...
Data collecting, Data processing, Classification) transposees sur le sujet livraison de repas. La mecanique
...
\texttt{notebook/meal\_delivery\_exercices.ipynb} sans
aide, chrono en main.
...
\begin{lstlisting}
courier = pd.read_csv(f"{DATA}/couriers.csv")
delivery = pd.concat([pd.read_csv(f"{DATA}/deliveries_{i}.csv") for i in range(1, 5)], ignore_index=True)

average_rating = courier["rating"].mean()
pct_second_language = (courier["second_language"] != "no").mean() * 100
success_rate = (delivery["status"] == "Success").mean() * 100
\end{lstlisting}
...
\begin{trap}
\begin{itemize}[nosep]
  \item Oublier de combiner les 4 fichiers \texttt{deliveries\_i.csv} avant
...
couriers.csv, vehicles.csv et 4 fichiers
d'deliverys, et vous devez reconstruire une seule table
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Sommer plusieurs colonnes bool\'eennes en une ligne} ->
        ...
        \texttt{True} par ligne ; agr\'egez ensuite par courier avec
        \texttt{groupby().sum()}.
  ...
  \item \textbf{fillna(0) apr\`es un merge sur une agr\'egation} -- un
        courier sans aucun \'ev\'enement n'a pas de ligne dans la
...
\begin{lstlisting}
upvote_cols = ['food_temperature_upvote_given', 'politeness_upvote_given', 'packaging_upvote_given', 'punctuality_upvote_given']
delivery["n_upvotes"] = delivery[upvote_cols].sum(axis=1)
upvotes_per_courier = delivery.groupby("courier\_id")["n_upvotes"].sum()

vehicles["last_inspection_date"] = pd.to_datetime(vehicles["last_inspection_date"])
vehicles["days_since_inspection"] = (TODAY - vehicles["last_inspection_date"]).dt.days
\end{lstlisting}
...
\begin{itemize}[nosep]
  ...
  \item Oublier \texttt{fillna(0)} sur \texttt{number\_of\_upvotes} apr\`es
        le merge -- les couriers sans \'ev\'enement se retrouvent avec
