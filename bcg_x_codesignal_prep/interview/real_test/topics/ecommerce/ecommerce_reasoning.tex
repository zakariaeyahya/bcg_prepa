...
\rhead{\small BCG X CodeSignal Prep}\lhead{\small E-commerce}
\cfoot{\thepage}
...
\begin{center}
  {\huge\bfseries\color{bcg} E-commerce}\\[4pt]
...
Data collecting, Data processing, Classification) transposees sur le sujet e-commerce. La mecanique
...
\texttt{notebook/ecommerce\_exercices.ipynb} sans
aide, chrono en main.
...
\begin{lstlisting}
seller = pd.read_csv(f"{DATA}/sellers.csv")
order = pd.concat([pd.read_csv(f"{DATA}/orders_{i}.csv") for i in range(1, 5)], ignore_index=True)

average_rating = seller["rating"].mean()
pct_second_language = (seller["second_language"] != "no").mean() * 100
success_rate = (order["status"] == "Success").mean() * 100
\end{lstlisting}
...
\begin{trap}
\begin{itemize}[nosep]
  \item Oublier de combiner les 4 fichiers \texttt{orders\_i.csv} avant
...
sellers.csv, warehouses.csv et 4 fichiers
d'orders, et vous devez reconstruire une seule table
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Sommer plusieurs colonnes bool\'eennes en une ligne} ->
        ...
        \texttt{True} par ligne ; agr\'egez ensuite par seller avec
        \texttt{groupby().sum()}.
  ...
  \item \textbf{fillna(0) apr\`es un merge sur une agr\'egation} -- un
        seller sans aucun \'ev\'enement n'a pas de ligne dans la
...
\begin{lstlisting}
upvote_cols = ['item_quality_upvote_given', 'communication_upvote_given', 'packaging_upvote_given', 'shipping_speed_upvote_given']
order["n_upvotes"] = order[upvote_cols].sum(axis=1)
upvotes_per_seller = order.groupby("seller\_id")["n_upvotes"].sum()

warehouses["last_inspection_date"] = pd.to_datetime(warehouses["last_inspection_date"])
warehouses["days_since_inspection"] = (TODAY - warehouses["last_inspection_date"]).dt.days
\end{lstlisting}
...
\begin{itemize}[nosep]
  ...
  \item Oublier \texttt{fillna(0)} sur \texttt{number\_of\_upvotes} apr\`es
        le merge -- les sellers sans \'ev\'enement se retrouvent avec
