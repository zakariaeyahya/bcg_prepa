...
\rhead{\small BCG X CodeSignal Prep}\lhead{\small Location courte duree}
\cfoot{\thepage}
...
\begin{center}
  {\huge\bfseries\color{bcg} Location courte duree}\\[4pt]
...
Data collecting, Data processing, Classification) transposees sur le sujet location courte duree. La mecanique
...
\texttt{notebook/short\_term\_rental\_exercices.ipynb} sans
aide, chrono en main.
...
\begin{lstlisting}
host = pd.read_csv(f"{DATA}/hosts.csv")
booking = pd.concat([pd.read_csv(f"{DATA}/bookings_{i}.csv") for i in range(1, 5)], ignore_index=True)

average_rating = host["rating"].mean()
pct_second_language = (host["second_language"] != "no").mean() * 100
success_rate = (booking["status"] == "Success").mean() * 100
\end{lstlisting}
...
\begin{trap}
\begin{itemize}[nosep]
  \item Oublier de combiner les 4 fichiers \texttt{bookings\_i.csv} avant
...
hosts.csv, properties.csv et 4 fichiers
d'bookings, et vous devez reconstruire une seule table
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Sommer plusieurs colonnes bool\'eennes en une ligne} ->
        ...
        \texttt{True} par ligne ; agr\'egez ensuite par host avec
        \texttt{groupby().sum()}.
  ...
  \item \textbf{fillna(0) apr\`es un merge sur une agr\'egation} -- un
        host sans aucun \'ev\'enement n'a pas de ligne dans la
...
\begin{lstlisting}
upvote_cols = ['cleanliness_upvote_given', 'communication_upvote_given', 'check_in_upvote_given', 'accuracy_upvote_given']
booking["n_upvotes"] = booking[upvote_cols].sum(axis=1)
upvotes_per_host = booking.groupby("host\_id")["n_upvotes"].sum()

properties["last_inspection_date"] = pd.to_datetime(properties["last_inspection_date"])
properties["days_since_inspection"] = (TODAY - properties["last_inspection_date"]).dt.days
\end{lstlisting}
...
\begin{itemize}[nosep]
  ...
  \item Oublier \texttt{fillna(0)} sur \texttt{number\_of\_upvotes} apr\`es
        le merge -- les hosts sans \'ev\'enement se retrouvent avec
